# Contributing to SMEMaster

Thank you for your interest in contributing to SMEMaster! This guide will help you get started.

## Getting Started

### Prerequisites

- [Node.js](https://nodejs.org/) v20 LTS or v22 LTS (v25+ is not yet supported by `@tauri-apps/cli`)
- [Rust](https://www.rust-lang.org/tools/install) (1.77.2+ stable)
- [Tauri v2 prerequisites](https://v2.tauri.app/start/prerequisites/)
- **Windows (GNU toolchain):** `C:\msys64\ucrt64\bin` must be in PATH (provides `dlltool.exe`)
...
Use the [bug report template](https://github.com/Zakarialabib/smeMaster/issues/new?template=bug_report.yml) on GitHub Issues. Include:

- Steps to reproduce
- Expected vs. actual behavior
- OS and SMEMaster version
- Screenshots or logs if applicable

## Feature Requests

Use the [feature request template](https://github.com/Zakarialabib/smeMaster/issues/new?template=feature_request.yml) on GitHub Issues.
...
To publish a new release to a Fedora COPR repository using the `smemaster.spec` file:
...
2.  **Create a Source Tarball and SRPM**
    ...
    Create a source tarball that matches the version in `smemaster.spec`, then build the SRPM.

    ```bash
    VERSION=$(grep -oP '(?<=^%global app_version ).*' smemaster.spec)
    tar --exclude='.git' --transform "s/^\./smemaster-${VERSION}/" -czf "smemaster-${VERSION}.tar.gz" .
    
    cp "smemaster-${VERSION}.tar.gz" ~/rpmbuild/SOURCES/
    cp smemaster.spec ~/rpmbuild/SPECS/
    
    rpmbuild -bs ~/rpmbuild/SPECS/smemaster.spec
    ```
...
3.  **Upload to COPR**
    ...
    ```bash
    copr build your-username/smemaster ~/rpmbuild/SRPMS/smemaster-${VERSION}-1.*.src.rpm
    ```
