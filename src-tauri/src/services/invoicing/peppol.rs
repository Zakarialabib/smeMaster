...
pub fn generate_peppol_xml(
    ...
    let buyer_name = contact.as_ref().and_then(|c| c.display_name.clone()).unwrap_or_else(|| "Customer".to_string());
    let buyer_email = contact.as_ref().map(|c| c.email.clone()).unwrap_or_else(|| "unknown@example.com".to_string());
    ...
    xml.push_str(&format!("  <cbc:ProfileID>urn:fdc:peppol.eu:poacc:bis:billing:3</cbc:ProfileID>\n"));
    ...
    xml.push_str(&format!("  <cbc:InvoiceTypeCode>380</cbc:InvoiceTypeCode>\n"));
    xml.push_str(&format!("  <cbc:DocumentCurrencyCode>{}</cbc:DocumentCurrencyCode>\n", invoice.currency));

    // Seller
    xml.push_str("  <cac:AccountingSupplierParty>\n");
    xml.push_str("    <cac:Party>\n");
    xml.push_str(&format!("      <cac:PartyName><cbc:Name>{}</cbc:Name></cac:PartyName>\n", company.name));
    xml.push_str("      <cac:PostalAddress>\n");
    if let Some(line1) = &company.address_line1 {
        xml.push_str(&format!("        <cbc:StreetName>{}</cbc:StreetName>\n", line1));
    }
    ...
    if let Some(country) = &company.country {
        ...
        xml.push_str(&format!("          <cbc:IdentificationCode>{}</cbc:IdentificationCode>\n", country));
        xml.push_str("        </cac:Country>\n");
    }
    xml.push_str("      </cac:PostalAddress>\n");
    ...
    if company.ice.is_some() || company.tax_id.is_some() || company.rc.is_some() {
        ...
        xml.push_str(&format!("        <cbc:RegistrationName>{}</cbc:RegistrationName>\n", company.legal_name.as_ref().unwrap_or(&company.name)));
        if let Some(ice) = &company.ice {
            xml.push_str(&format!("        <cbc:CompanyID schemeID=\"ICE\">{}</cbc:CompanyID>\n", ice));
        }
        if let Some(tax_id) = &company.tax_id {
            xml.push_str(&format!("        <cbc:CompanyID schemeID=\"IF\">{}</cbc:CompanyID>\n", tax_id));
        }
        if let Some(rc) = &company.rc {
            xml.push_str(&format!("        <cbc:CompanyID schemeID=\"RC\">{}</cbc:CompanyID>\n", rc));
        }
        xml.push_str("      </cac:PartyLegalEntity>\n");
    }
    ...
    xml.push_str(&format!("        <cbc:ElectronicMail>{}</cbc:ElectronicMail>\n", buyer_email));
    xml.push_str("      </cac:Contact>\n");
    xml.push_str("    </cac:Party>\n");
    xml.push_str("  </cac:AccountingCustomerParty>\n");
    ...
    xml.push_str(&format!("    <cbc:LineExtensionAmount currencyID=\"{}\">{:.2}</cbc:LineExtensionAmount>\n", invoice.currency, invoice.subtotal));
    xml.push_str(&format!("    <cbc:TaxExclusiveAmount currencyID=\"{}\">{:.2}</cbc:TaxExclusiveAmount>\n", invoice.currency, invoice.subtotal));
    xml.push_str(&format!("    <cbc:TaxInclusiveAmount currencyID=\"{}\">{:.2}</cbc:TaxInclusiveAmount>\n", invoice.currency, invoice.total_amount));
    xml.push_str(&format!("    <cbc:PayableAmount currencyID=\"{}\">{:.2}</cbc:PayableAmount>\n", invoice.currency, invoice.total_amount));
    xml.push_str("  </cac:LegalMonetaryTotal>\n");
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        xml.push_str(&format!("    <cbc:ID>{}</cbc:ID>\n", idx + 1));
        xml.push_str(&format!("    <cbc:InvoicedQuantity unitCode=\"ZZ\">{:.2}</cbc:InvoicedQuantity>\n", item.quantity));
        xml.push_str(&format!("    <cbc:LineExtensionAmount currencyID=\"{}\">{:.2}</cbc:LineExtensionAmount>\n", invoice.currency, item.quantity * item.unit_price));
        ...
        xml.push_str(&format!("        <cbc:Percent>{:.2}</cbc:Percent>\n", item.tax_rate));
        ...
        xml.push_str(&format!("      <cbc:PriceAmount currencyID=\"{}\">{:.2}</cbc:PriceAmount>\n", invoice.currency, item.unit_price));
        xml.push_str("    </cac:Price>\n");
        xml.push_str("  </cac:InvoiceLine>\n");
    }
