...
| **Local embeddings + local vector store + optional cloud LLM** | ✅ **Adopted** | Embeddings run locally (candle), vectors stored locally (LanceDB), only the *retrieved context* is sent to the cloud LLM when the user opts in. |
...
│  ragStore · RagSearchBar · RagResultBubble · RagStatusBadge   │
...
| Frontend | RAG state | `src/features/assistant/store/ragStore.ts` |
...
### Indexing
1. User clicks "Index Emails" (`LocalRagSettings` or `RagStatusBadge`).
...
3. UI updates `RagStatusBadge` (indexed count / ready state).
...
| Store | `src/features/assistant/store/ragStore.ts` |
| Components | `src/features/assistant/components/` (`RagSearchBar.tsx`, `RagResultBubble.tsx`, `RagStatusBadge.tsx`, `AiAssistantPage.tsx`) |
