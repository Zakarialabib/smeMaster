...
vi.mock("@features/mail/components/composer/EditorToolbar", () => ({
  EditorToolbar: vi.fn(() => <div data-testid="editor-toolbar">Toolbar</div>),
}));

vi.mock("@shared/components/ui/TextField", () => ({
  TextField: vi.fn((props: React.InputHTMLAttributes<HTMLInputElement>) => (
    <input {...props} />
  )),
}));

const mockGetSignatures = vi.fn<() => Promise<import("@features/mail/db/signatures").DbSignature[]>>().mockResolvedValue([]);
...
vi.mock("@features/mail/db/signatures", () => ({
