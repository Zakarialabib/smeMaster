...
export function TemplateManager() {
  ...
  const maxUsageCount = useMemo(() => {
    if (paginatedTemplates.length === 0) return 1;
    return Math.max(...paginatedTemplates.map((t) => t.usage_count), 1);
  }, [paginatedTemplates]);
  ...
  const confirmImport = useCallback(async () => {
    ...
    const existingNames = new Set(paginatedTemplates.map((t) => t.name.toLowerCase()));
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {paginatedTemplates.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-text-tertiary whitespace-nowrap">
                {paginatedTemplates.length}/10 used
              </span>
          ...
          {!canCreateTemplate && paginatedTemplates.length > 0 && (
            <UpgradeBadge
              variant="limit"
              message="Limit reached"
              tooltipContent={`You've reached the maximum of 10 paginatedTemplates on the Basic plan. Upgrade to Pro for unlimited paginatedTemplates.`}
            />
          )}
        </div>
      </div>
      ...
      {paginatedTemplates.length > 0 && (
        <TemplateAnalytics paginatedTemplates={paginatedTemplates} maxUsage={maxUsageCount} />
      )}
    </div>
  );
}
