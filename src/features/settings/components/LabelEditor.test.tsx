...
const mockLabelsState: { current: Record<string, unknown>[] } = { current: [] };

vi.mock("@features/mail/hooks/useLabels", () => ({
  useLabels: () => ({ data: mockLabelsState.current, isLoading: false }),
}));
...
function setStoreWithLabels(labels: { id: string; accountId: string; name: string; type: string; colorBg: string | null; colorFg: string | null; sortOrder: number }[]) {
  mockLabelsState.current = labels;
...
function renderWithQuery(ui: React.JSX.Element) {
  return render(ui);
}
