...
vi.mock("@features/mail/services/snooze/snoozeManager", () => ({
  snoozeThread: vi.fn(() => Promise.resolve()),
}));

const mockUpdateThread = vi.fn();
const mockRemoveThreads = vi.fn();
...
vi.mock("@features/mail/stores/threadStore", () => ({
  useThreadStore: {
    getState: () => ({
      ...
      removeThreads: mockRemoveThreads,
    }),
    setState: vi.fn(),
    subscribe: vi.fn(),
    destroy: vi.fn(),
...
import { pinThread, unpinThread } from "@shared/services/db/threads";
