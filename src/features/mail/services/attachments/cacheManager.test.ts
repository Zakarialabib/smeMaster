...
const { mockExecuteSearchQuery, mockCacheAttachmentDb, mockEvictSingleAttachmentCache, mockClearAttachmentCache } = vi.hoisted(() => ({
  mockExecuteSearchQuery: vi.fn(),
  mockCacheAttachmentDb: vi.fn(),
  mockEvictSingleAttachmentCache: vi.fn(),
  mockClearAttachmentCache: vi.fn(),
}));

vi.mock("@shared/services/db/db-invoke", () => ({
  executeSearchQuery: mockExecuteSearchQuery,
  cacheAttachmentDb: mockCacheAttachmentDb,
  evictSingleAttachmentCache: mockEvictSingleAttachmentCache,
  clearAttachmentCache: mockClearAttachmentCache,
}));
...
describe("cacheManager", () => {
  ...
  describe("cacheAttachment", () => {
    ...
    it("updates DB via cacheAttachmentDb wrapper", async () => {
      const data = new Uint8Array([10, 20]);
      await cacheAttachment("att-1", data);

      expect(mockCacheAttachmentDb).toHaveBeenCalledWith(
        "att-1",
        expect.stringContaining("attachment_cache/"),
        expect.any(Number),
      );
  ...
  describe("getCacheSize", () => {
    it("returns total cache size from DB", async () => {
      mockExecuteSearchQuery.mockResolvedValueOnce([{ total: 1024 }]);
      const size = await getCacheSize();
      expect(size).toBe(1024);
    });

    it("returns 0 when no cached attachments", async () => {
      mockExecuteSearchQuery.mockResolvedValueOnce([{ total: 0 }]);
      const size = await getCacheSize();
      expect(size).toBe(0);
    });
  });
  ...
  describe("evictOldestCached", () => {
    it("removes files with baseDir option when over limit", async () => {
      ...
      expect(mockEvictSingleAttachmentCache).toHaveBeenCalledWith("att-old");
    });

    it("does nothing when under limit", async () => {
      mockExecuteSearchQuery.mockResolvedValueOnce([{ total: 100 }]);
  ...
  describe("clearAllCache", () => {
    ...
    it("clears cache via clearAttachmentCache wrapper", async () => {
      await clearAllCache();

      expect(mockClearAttachmentCache).toHaveBeenCalled();
    });
  });
});
