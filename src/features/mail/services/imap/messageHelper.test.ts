...
const { mockExecuteSearchQuery, mockBulkUpdateMessageImapFolder } = vi.hoisted(() => ({
  mockExecuteSearchQuery: vi.fn(),
  mockBulkUpdateMessageImapFolder: vi.fn(),
}));

vi.mock("@shared/services/db/db-invoke", () => ({
  executeSearchQuery: mockExecuteSearchQuery,
  bulkUpdateMessageImapFolder: mockBulkUpdateMessageImapFolder,
}));
...
describe("messageHelper", () => {
  ...
  describe("findSpecialFolder", () => {
    it("returns null when no matching folder exists", async () => {
      mockExecuteSearchQuery.mockResolvedValue([]);
  ...
  describe("updateMessageImapFolder", () => {
    it("does nothing for empty message list", async () => {
      ...
      expect(mockBulkUpdateMessageImapFolder).not.toHaveBeenCalled();
    });
    ...
    it("updates folder for given messages", async () => {
      ...
      expect(mockBulkUpdateMessageImapFolder).toHaveBeenCalledTimes(1);
      expect(mockBulkUpdateMessageImapFolder).toHaveBeenCalledWith(
        "acc1",
        ["msg1", "msg2"],
        "Trash",
      );
