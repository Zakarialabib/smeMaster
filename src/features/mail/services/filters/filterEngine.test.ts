...
import type { FilterCriteria, FilterActions } from "@features/mail/db/filters";
import { createMockParsedMessage } from "@/test/mocks";

vi.mock("@features/mail/db/filters", async () => {
  const actual = await vi.importActual("@features/mail/db/filters");
  return {
    ...actual,
...
describe("evaluateChainedRules", () => {
  async function mockConditions(map: Record<string, any[]>) {
    const { getFilterConditionsForRule } = await import("@features/mail/db/filters");
