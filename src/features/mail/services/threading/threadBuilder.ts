 ...
 * Used for fuzzy subject matching when exact match fails.
 */
...
/**
 * Check if two normalized subjects should be considered a fuzzy match.
 * Uses Levenshtein distance relative to subject length.
 */
export function subjectsMatchFuzzy(a: string, b: string, threshold = 0.3): boolean {
  if (!a || !b) return false;
  if (a === b) return true;
...
/**
 * Merge two root containers by subject, preferring phantom containers
 * as the root and otherwise keeping the older message's container.
 */
function mergeRoots(existing: Container, root: Container): void {
...
export function buildThreads(messages: ThreadableMessage[]): ThreadGroup[] {
  ...
  // Step 4: Group by subject — merge roots with same or fuzzy-matching normalized subject
  const subjectMap = new Map<string, Container>();
  const fuzzyBins: { normalized: string; container: Container }[] = [];
  const FUZZY_THRESHOLD = 0.3;
  const TIME_WINDOW = 2 * 24 * 3600; // 2 days in seconds — window for fuzzy merging
  ...
  for (const root of roots) {
    ...
    if (!existing) {
      // Try fuzzy match against existing exact keys first
      let fuzzyMatched = false;
      for (const [key, existingContainer] of subjectMap) {
        if (subjectsMatchFuzzy(normalized, key, FUZZY_THRESHOLD)) {
          // Also check time window: only merge if within 2 days
          const myDate = root.message?.date ?? 0;
          const existingDate = existingContainer.message?.date ?? 0;
          if (Math.abs(myDate - existingDate) <= TIME_WINDOW) {
            mergeRoots(existingContainer, root);
            fuzzyMatched = true;
            break;
          }
        }
      ...
      if (!fuzzyMatched) {
        ...
        fuzzyBins.push({ normalized, container: root });
      }
    } else {
      mergeRoots(existing, root);
    }
  }
