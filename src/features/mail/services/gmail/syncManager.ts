    ...
    for (const cal of visibleCals) {
      try {
        ...
        for (const remoteId of syncResult.deletedRemoteIds) {
          await deleteEventByRemoteId(accountId, cal.id, remoteId);
        }
...
async function runSync(accountIds: string[]): Promise<void> {
  ...
  syncPromise = (async () => {
    ...
    // Drain the queue â€” if something was queued while we were syncing, run it now
 ...
 * next interval tick â€” useful when the caller already triggered a sync for a
