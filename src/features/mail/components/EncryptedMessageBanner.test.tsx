...
vi.mock("@shared/services/pgp/pgpService", () => ({
...
vi.mock("@shared/services/pgp/passphraseCache", () => ({
  getCachedPassphrase: vi.fn().mockResolvedValue(null),
  cachePassphrase: vi.fn().mockResolvedValue(undefined),
  clearPassphraseCache: vi.fn().mockResolvedValue(undefined),
}));
...
describe("EncryptedMessageBanner", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const pgp = await import("@shared/services/pgp/pgpService");
    ...
    const pgpCache = await import("@shared/services/pgp/passphraseCache");
    vi.mocked(pgpCache.getCachedPassphrase).mockResolvedValue(null);
    vi.mocked(pgpCache.cachePassphrase).mockResolvedValue(undefined);
  });
  ...
  it("shows cached passphrase state when passphrase is cached", async () => {
    const pgp = await import("@shared/services/pgp/passphraseCache");
    vi.mocked(pgp.getCachedPassphrase).mockResolvedValue("cached-pass");
    render(<EncryptedMessageBanner {...defaultProps} />);
  ...
  it("shows loading state during decryption", async () => {
    const pgp = await import("@shared/services/pgp/pgpService");
    // beforeEach already sets getCachedPassphrase to mockResolvedValue(null)
