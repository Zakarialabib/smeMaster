...
export function RawMessageModal({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={t('modals.rawMessage.title')}
      width="w-[720px] max-w-[90vw]"
      renderHeader={
        <div className="px-4 py-3 border-b border-border-primary flex items-center justify-between">
          <h3 className="text-sm font-semibold text-text-primary">
            {t('modals.rawMessage.title')}
          </h3>
          <div className="flex items-center gap-2">
            {raw && (
              <button
                ...
                title={t('modals.rawMessage.copyToClipboard')}
              >
                {copied ? <Check size={14} /> : <Copy size={14} />}
                {copied ? t('modals.rawMessage.copied') : t('modals.rawMessage.copy')}
              </button>
            )}
            <button
              onClick={onClose}
      ...
      <div className="max-h-[70vh] overflow-y-auto p-4">
        {loading && (
          <div className="flex items-center justify-center py-12 text-text-tertiary text-sm">
            {t('modals.rawMessage.loading')}
          </div>
        )}
        {error && (
          <div className="text-danger text-sm py-4">
            {t('modals.rawMessage.failedToLoad', { error })}
          </div>
        )}
        {raw && (
