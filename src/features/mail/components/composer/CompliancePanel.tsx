...
import { detectJurisdiction, evaluateRules, aggregateResults } from "@features/mail/services/compliance/ruleEngine";
import { getActiveProfiles, insertCheck } from "@features/mail/db/complianceProfiles";
import type { ComplianceViolation, ComplianceCheckContext } from "@features/mail/services/compliance/types";
...
export function CompliancePanel({ editor, accountId, subject, bodyHtml, recipients, onViolationsChange }: CompliancePanelProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      insertCheck(
        ...
        null,
        null,
        matched.map((p) => p.id).join(","),
        aggregated.score,
        JSON.stringify(aggregated.violations),
      ).catch(() => {});
    }, 500);
  ...
  const sv = getScoreVariant(score);
  ...
  return (
    <div className={`mx-3 my-1.5 px-3 py-1.5 rounded-lg border ${sv.bgColor} border-border-secondary flex items-center gap-2 relative`}>
      <ScoreIcon size={14} className={`shrink-0 ${sv.color}`} />
      <span className={`text-xs font-medium ${sv.color}`}>
        {score}/100
      </span>
      {violations.length > 0 && (
        <button
