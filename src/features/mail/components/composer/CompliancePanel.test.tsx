...
vi.mock("@features/mail/services/compliance/ruleEngine", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@features/mail/services/compliance/ruleEngine")>();
  return {
    ...actual,
...
vi.mock("@features/mail/db/complianceProfiles", () => ({
...
describe("CompliancePanel", () => {
  ...
  it("shows orange badge for score < 70 (fair range)", async () => {
    render(<CompliancePanel {...defaultProps} />);
    const scoreText = await screen.findByText(/60\/100/);
    expect(scoreText.className).toContain("text-orange-500");
  });

  it("shows green badge for score > 90", async () => {
    const ruleEngine = await import("@features/mail/services/compliance/ruleEngine");
