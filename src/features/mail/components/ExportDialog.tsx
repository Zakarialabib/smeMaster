...
export function ExportDialog({ accountId, isOpen, onClose }: ExportDialogProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t('modals.export.title')} width="w-[520px]">
      <div className="p-6 space-y-6">
        {done ? (
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 px-3 py-2 text-xs bg-bg-tertiary border border-border-primary rounded-md text-text-secondary truncate">
                {destinationPath || t('modals.export.noFileSelected')}
              </div>
              <button
          ...
          <>
            <p className="text-sm text-text-secondary">{t('modals.export.reviewConfirm')}</p>
            <div className="space-y-2 p-3 bg-bg-tertiary rounded-lg">
              <div className="flex items-center justify-between text-xs">
                <span className="text-text-tertiary">{t('modals.export.format')}</span>
              ...
              {dateFrom && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-text-tertiary">{t('modals.export.from')}</span>
                  <span className="text-text-primary">{dateFrom}</span>
                </div>
              )}
              {dateTo && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-text-tertiary">{t('modals.export.to')}</span>
