...
import { Paperclip, Search, LayoutGrid, List, SlidersHorizontal, X, AlertCircle, RefreshCw } from "lucide-react";
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={isMobileDevice ? "pl-8 pr-3 py-1.5 text-xs rounded-md border border-border-primary bg-bg-secondary text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-1 focus:ring-accent w-32" : "pl-8 pr-3 py-1.5 text-xs rounded-md border border-border-primary bg-bg-secondary text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-1 focus:ring-accent w-48"}
            />
          </div>
      ...
      {isMobileDevice && showFilters && (
        <div className="flex flex-wrap gap-2 px-4 py-2 border-b border-border-primary bg-bg-primary/40">
          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value as TypeFilter)}
            className="flex-1 min-w-0 text-xs rounded-md border border-border-primary bg-bg-secondary text-text-primary px-2 py-1.5"
          >
            {TYPE_OPTIONS.map((o) => (
              <option key={o.value} value={o.value}>{o.label}</option>
            ))}
