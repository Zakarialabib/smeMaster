...
import { saveAttachmentToVault } from "@features/mail/services/attachments/vaultService";
import { Modal } from "@shared/components/ui/Modal";
import { Download, Eye, Archive } from "lucide-react";
import { formatFileSize, isImage, isPdf, isText, canPreview, getFileIcon } from "@shared/utils/fileTypeHelpers";
...
function LazyAttachmentItem({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!attachment.gmail_attachment_id) return;
        ...
        const blob = new Blob([bytes.buffer as ArrayBuffer], { type: attachment.mime_type ?? "image/png" });
        setThumbnailUrl(URL.createObjectURL(blob));
      } catch {
        // Silent fail - thumbnail preload is optional
      }
    })();
  }, [isVisible, attachment, accountId, messageId]);
  ...
  return (
    ...
    >
      {thumbnailUrl ? (
        ...
        <span className="text-text-tertiary">{getFileIcon(attachment.mime_type)}</span>
      )}
    ...
    <>
      <div className="mt-3 pt-3 border-t border-border-secondary">
        <div className="text-xs text-text-tertiary mb-2">
          {t('email.nAttachments', { n: fileAttachments.length })}
        </div>
    ...
    <div className="px-4 py-3 border-b border-border-primary flex items-center justify-between shrink-0">
      ...
      <div className="flex items-center gap-2 shrink-0 ml-4">
        {fromAddress && (
          ...
          >
            <Archive size={13} />
            {savingToVault ? t('common.saving') : vaultSaved ? t('common.saved') : t('vault.saveToVault')}
          </button>
        )}
        <button
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-auto min-h-[200px] flex items-center justify-center p-4" data-native-context-menu>
        {loading && (
          <p className="text-sm text-text-tertiary">{t('common.loadingPreview')}</p>
        )}
        {error && (
        ...
        {!isPreviewable && !loading && (
          <div className="flex flex-col items-center gap-3 text-text-tertiary">
            ...
            <p className="text-xs">{attachment.mime_type ?? t('common.unknownType')}</p>
          </div>
        )}
      </div>
    </Modal>
...
function TextPreview({ url }: { url: string }) {
  ...
  useEffect(() => {
    fetch(url).then((r) => r.text()).then(setText).catch(() => setText(t('common.failedToLoadText')));
  }, [url, t]);
