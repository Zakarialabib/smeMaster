...
/**
 * Format an error with its code and provide actionable guidance.
 * Returns a user-friendly message with error code for debugging.
 */
function formatConnectionError(
  ...
  switch (code) {
    ...
    case ErrorCodes.TIMEOUT:
      return {
        code,
        message: `${serverType} connection to ${host}:${port} timed out after 15s — check the server address, port, and firewall settings. If using a local bridge (e.g., ProtonMail Bridge), ensure it's running.`,
      };

    case ErrorCodes.AUTH_FAILED:
      return {
        code,
        message: `${serverType} authentication failed — verify your username and password. For Gmail/Outlook, use an app-specific password instead of your regular password.`,
      };

    case ErrorCodes.NETWORK_ERROR:
      return {
        code,
        message: `${serverType} network error — could not connect to ${host}:${port}. Verify the server is reachable and your internet connection is stable.`,
      };

    case ErrorCodes.PARSE_ERROR:
      return {
        code,
        message: `${serverType} configuration error — ${message}. Check that the port is a valid number and security settings are correct.`,
      };

    case ErrorCodes.INTERNAL_ERROR:
      return {
        code,
        message: `${serverType} connection failed: ${message}`,
      };

    default:
      // Fallback for unknown codes - still show the code for debugging
      return {
        code,
        message: `${serverType} connection error [${code}]: ${message}`,
      };
  }
}
...
export function AddImapAccount({
  ...
  }, [currentStep, canAdvanceFromBasic, canAdvanceFromImap, canAdvanceFromSmtp]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const acc: Account = {
        ...
        company: null,
        avatarUrl: createdAccount.avatar_url,
        isActive: true,
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div key={step} className="flex items-center gap-1">
            ...
            >
              {stepIcons[step]}
              <span className="hidden sm:inline">{t(stepLabels[step])}</span>
            </div>
          </div>
        );
      })}
