...
export function EventDetailModal({ event, type = 'event', calendars, accountId, onClose, onUpdated }: EventDetailModalProps) {
    ...
    try {
      if (type === 'event') {
        ...
        await updateTask(event.id, {
          title: summary,
          description,
          dueDate: Math.floor(new Date(startTime).getTime() / 1000),
        });
      }
  ...
  }, [accountId, calendar, event, type, onUpdated]);
  ...
  const getTypeIcon = () => {
    ...
    if (type === 'campaign') return <Send size={14} />;
    if (type === 'scheduled_email') return <Mail size={14} />;
    return null;
  };
      ...
      <div className="p-4 space-y-3">
        {(calendar || type !== 'event') && (
          <div className="flex items-center gap-2 text-xs font-medium" style={{ color: getTypeColor() }}>
            ...
            {calendar && <span className="text-text-tertiary">· {calendar.display_name}</span>}
          </div>
        )}
        ...
        <div className="flex items-start gap-2.5 text-sm text-text-secondary">
          ...
          <div>
            ...
            {type === 'event' && <div>{formatTime(event.end_time)}</div>}
          </div>
        </div>
