...
export function ContactMergeDialog({ isOpen, onClose, candidates, onMerge }: ContactMergeDialogProps) {
  ...
  if (candidates.length === 0) {
    return (
      <Modal isOpen={isOpen} onClose={onClose} title={t('modals.contactMerge.title')} size="sm">
        <div className="p-4 text-sm text-text-tertiary text-center">
          {t('modals.contactMerge.noDuplicates')}
        </div>
      </Modal>
    );
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('modals.contactMerge.title')} size="lg">
