import { describe, it, expect, beforeEach, vi } from "vitest";
import { useThreadsStore, type Thread } from "@/stores/mail";

const mockGetAllLabelUnreadCounts = vi.fn();

vi.mock("@shared/services/db/threads", () => ({
  getAllLabelUnreadCounts: (...args: unknown[]) => mockGetAllLabelUnreadCounts(...args),
}));

const mockThread: Thread = {
  id: "thread-1",
  accountId: "acc-1",
  subject: "Test Subject",
  snippet: "This is a test...",
  lastMessageAt: 1700000000,
  messageCount: 3,
  isRead: false,
  isStarred: false,
  isPinned: false,
  isMuted: false,
  hasAttachments: false,
  labelIds: ["INBOX"],
  fromName: "John Doe",
  fromAddress: "john@example.com",
};

const mockThread2: Thread = {
  id: "thread-2",
  accountId: "acc-1",
  subject: "Another Thread",
  snippet: "Another preview...",
  lastMessageAt: 1700001000,
  messageCount: 1,
  isRead: true,
  isStarred: true,
  isPinned: false,
  isMuted: false,
  hasAttachments: true,
  labelIds: ["INBOX", "STARRED"],
  fromName: "Jane Smith",
  fromAddress: "jane@example.com",
};

describe("threadStore", () => {
  beforeEach(() => {
    useThreadsStore.setState({
      threads: [],
      threadMap: new Map(),
      selectedThreadId: null,
      selectedThreadIds: new Set(),
      isLoading: false,
    });
  });

  it("should start with empty threads", () => {
    const state = useThreadsStore.getState();
    expect(state.threads).toHaveLength(0);
    expect(state.selectedThreadId).toBeNull();
    expect(state.isLoading).toBe(false);
  });

  it("should set threads", () => {
    useThreadsStore.getState().setThreads([mockThread, mockThread2]);
    expect(useThreadsStore.getState().threads).toHaveLength(2);
  });

  it("should select a thread", () => {
    useThreadsStore.getState().setThreads([mockThread]);
    useThreadsStore.getState().selectThread("thread-1");
    expect(useThreadsStore.getState().selectedThreadId).toBe("thread-1");
  });

  it("should deselect a thread", () => {
    useThreadsStore.getState().selectThread("thread-1");
    useThreadsStore.getState().selectThread(null);
    expect(useThreadsStore.getState().selectedThreadId).toBeNull();
  });

  it("should set loading state", () => {
    useThreadsStore.getState().setLoading(true);
    expect(useThreadsStore.getState().isLoading).toBe(true);
  });

  it("should select all threads", () => {
    useThreadsStore.getState().setThreads([mockThread, mockThread2]);
    useThreadsStore.getState().selectAll();
    const state = useThreadsStore.getState();
    expect(state.selectedThreadIds.size).toBe(2);
    expect(state.selectedThreadIds.has("thread-1")).toBe(true);
    expect(state.selectedThreadIds.has("thread-2")).toBe(true);
  });

  it("should select all threads from the selected thread onward", () => {
    const mockThread3: Thread = {
      ...mockThread,
      id: "thread-3",
      subject: "Third Thread",
    };
    useThreadsStore.getState().setThreads([mockThread, mockThread2, mockThread3]);
    useThreadsStore.getState().selectThread("thread-2");
    useThreadsStore.getState().selectAllFromHere();
    const state = useThreadsStore.getState();
    // Should select thread-2 and thread-3 (from index 1 onward)
    expect(state.selectedThreadIds.size).toBe(2);
    expect(state.selectedThreadIds.has("thread-2")).toBe(true);
    expect(state.selectedThreadIds.has("thread-3")).toBe(true);
    expect(state.selectedThreadIds.has("thread-1")).toBe(false);
  });

  it("should select all from beginning when no thread is selected", () => {
    useThreadsStore.getState().setThreads([mockThread, mockThread2]);
    useThreadsStore.getState().selectAllFromHere();
    const state = useThreadsStore.getState();
    expect(state.selectedThreadIds.size).toBe(2);
  });

  it("should merge selectAllFromHere with existing selection", () => {
    const mockThread3: Thread = {
      ...mockThread,
      id: "thread-3",
      subject: "Third Thread",
    };
    useThreadsStore.getState().setThreads([mockThread, mockThread2, mockThread3]);
    // Select thread-2 as the current thread
    useThreadsStore.getState().selectThread("thread-2");
    // Manually add thread-1 to multi-select (after selectThread since it clears multiselect)
    useThreadsStore.getState().toggleThreadSelection("thread-1");
    // Now selectAllFromHere should merge with the existing selection
    useThreadsStore.getState().selectAllFromHere();
    const state = useThreadsStore.getState();
    // Should have thread-1 (from toggle) + thread-2, thread-3 (from selectAllFromHere)
    expect(state.selectedThreadIds.size).toBe(3);
  });

  describe("threadMap", () => {
    it("should build threadMap when setting threads", () => {
      useThreadsStore.getState().setThreads([mockThread, mockThread2]);
      const { threadMap } = useThreadsStore.getState();
      expect(threadMap.size).toBe(2);
      expect(threadMap.get("thread-1")).toBe(useThreadsStore.getState().threads[0]);
      expect(threadMap.get("thread-2")).toBe(useThreadsStore.getState().threads[1]);
    });

    it("should return undefined for non-existent thread in threadMap", () => {
      useThreadsStore.getState().setThreads([mockThread]);
      expect(useThreadsStore.getState().threadMap.get("non-existent")).toBeUndefined();
    });

    it("should update threadMap when updating a thread", () => {
      useThreadsStore.getState().setThreads([mockThread, mockThread2]);
      useThreadsStore.getState().updateThread("thread-1", { isRead: true });
      const { threadMap } = useThreadsStore.getState();
      expect(threadMap.get("thread-1")?.isRead).toBe(true);
      expect(threadMap.get("thread-2")?.isRead).toBe(true); // was already true
    });

    it("should remove from threadMap when removing a thread", () => {
      useThreadsStore.getState().setThreads([mockThread, mockThread2]);
      useThreadsStore.getState().removeThread("thread-1");
      const { threadMap } = useThreadsStore.getState();
      expect(threadMap.size).toBe(1);
      expect(threadMap.has("thread-1")).toBe(false);
      expect(threadMap.has("thread-2")).toBe(true);
    });

    it("should remove from threadMap when removing multiple threads", () => {
      const mockThread3: Thread = { ...mockThread, id: "thread-3" };
      useThreadsStore.getState().setThreads([mockThread, mockThread2, mockThread3]);
      useThreadsStore.getState().removeThreads(["thread-1", "thread-3"]);
      const { threadMap } = useThreadsStore.getState();
      expect(threadMap.size).toBe(1);
      expect(threadMap.has("thread-2")).toBe(true);
    });

    it("should start with empty threadMap", () => {
      expect(useThreadsStore.getState().threadMap.size).toBe(0);
    });
  });

  describe("unreadCounts", () => {
    beforeEach(() => {
      vi.clearAllMocks();
    });

    it("should start with empty unreadCounts", () => {
      const { unreadCounts } = useThreadsStore.getState();
      expect(unreadCounts).toEqual({});
    });

    it("should set unreadCounts via loadUnreadCounts", async () => {
      mockGetAllLabelUnreadCounts.mockResolvedValueOnce({
        INBOX: 5,
        "label-1": 3,
        "label-2": 0,
      });

      await useThreadsStore.getState().loadUnreadCounts("acc-1");

      const { unreadCounts } = useThreadsStore.getState();
      expect(unreadCounts).toEqual({
        INBOX: 5,
        "label-1": 3,
        "label-2": 0,
      });
    });

    it("should be a record of labelId -> number", async () => {
      mockGetAllLabelUnreadCounts.mockResolvedValueOnce({
        INBOX: 10,
      });

      await useThreadsStore.getState().loadUnreadCounts("acc-1");

      const { unreadCounts } = useThreadsStore.getState();
      expect(typeof unreadCounts).toBe("object");
      expect(unreadCounts["INBOX"]).toBe(10);
      expect(typeof unreadCounts["INBOX"]).toBe("number");
    });

    it("should pass the accountId to getAllLabelUnreadCounts", async () => {
      mockGetAllLabelUnreadCounts.mockResolvedValueOnce({});

      await useThreadsStore.getState().loadUnreadCounts("acc-42");

      expect(mockGetAllLabelUnreadCounts).toHaveBeenCalledWith("acc-42");
    });
  });

  it("should update a specific thread", () => {
    useThreadsStore.getState().setThreads([mockThread, mockThread2]);
    useThreadsStore.getState().updateThread("thread-1", { isRead: true, isStarred: true });

    const updated = useThreadsStore.getState().threads.find((t) => t.id === "thread-1");
    expect(updated?.isRead).toBe(true);
    expect(updated?.isStarred).toBe(true);
    expect(updated?.subject).toBe("Test Subject"); // unchanged

    // Other thread should be untouched
    const other = useThreadsStore.getState().threads.find((t) => t.id === "thread-2");
    expect(other?.isRead).toBe(true); // was already true
  });
});
