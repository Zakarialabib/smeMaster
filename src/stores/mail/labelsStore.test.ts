...
import { useLabelsStore, isSystemLabel } from "@/stores/mail";
...
import { getLabelsForAccount, deleteLabel as dbDeleteLabel, updateLabelSortOrder, upsertLabel } from "@shared/services/db/labels";
...
describe("labelStore", () => {
  beforeEach(() => {
    useLabelsStore.setState({ labels: [], isLoading: false });
    vi.clearAllMocks();
  });
  ...
  it("should load labels and filter out system labels", async () => {
    ...
    await useLabelsStore.getState().loadLabels("acc1");
  ...
  it("should create a label via Gmail API and update DB", async () => {
    ...
    await useLabelsStore.getState().createLabel("acc1", "New Label", { textColor: "#ffffff", backgroundColor: "#fb4c2f" });
  ...
  it("should update a label via Gmail API and update DB", async () => {
    ...
    await useLabelsStore.getState().updateLabel("acc1", "Label_1", {
      name: "Renamed",
  ...
  it("should delete a label via Gmail API and DB", async () => {
    ...
    await useLabelsStore.getState().deleteLabel("acc1", "Label_1");
  ...
  it("should reorder labels by updating sort order in DB", async () => {
    ...
    await useLabelsStore.getState().reorderLabels("acc1", ["Label_2", "Label_1", "Label_3"]);
