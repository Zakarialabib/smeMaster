...
setSyncingFolder: (isSyncingFolder) => set({ isSyncingFolder }),
   ...
   * Routes sync lifecycle events to local state.
   */
  handleEvent: (eventType: string, _payload: unknown) => {
    switch (eventType) {
      case "sync:started":
      ...
      case "rust:init:complete":
        // Backend finished initializing — front-end is already online,
        // but we treat this as a signal that the Rust side is ready.
        // No state change needed by default, but mark online.
        set({ isOnline: true });
        break;
    }
  },
...
// ---------------------------------------------------------------------------
// Sub-store re-exports for incremental migration
// ---------------------------------------------------------------------------
// Consumers can gradually migrate from useUIStore to the domain-specific stores:
//
//   useUIStore(s => s.theme)              → useThemeStore(s => s.theme)
//   useUIStore(s => s.sidebarCollapsed)   → useLayoutStore(s => s.sidebarCollapsed)
//   useUIStore(s => s.isOnline)           → useSyncStore(s => s.isOnline)
//   useUIStore(s => s.setTheme)           → useThemeStore(s => s.setTheme)
//   useUIStore.getState().theme           → useThemeStore.getState().theme
//
export { useThemeStore } from "./themeStore";
export { useLayoutStore } from "./layoutStore";
export { useSyncStore } from "./syncStore";

// Note: All types are already exported directly from this file (FontScale,
// ReadingPanePosition, EmailDensity, etc.) and/or from their canonical sources.
// No additional type re-exports are needed.
