...
export interface ClassifiedError {
  type: "auth" | "network" | "quota" | "server" | "config" | "permanent";
  isRetryable: boolean;
  message: string;
}
...
export function classifyError(error: unknown): ClassifiedError {
  ...
  // Check configuration error patterns
  if (CONFIG_PATTERNS.some((pattern) => lower.includes(pattern))) {
    return { type: "config", isRetryable: false, message };
  }
