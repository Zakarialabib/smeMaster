  ...
  switch (actionId) {
    ...
    case "nav.escape": {
      if (useComposerStore.getState().isOpen) {
        ...
        useThreadsStore.getState().clearMultiSelect();
      } else if (selectedId) {
        navigateBack();
      }
      break;
      ...
      if (multiMuteIds.size > 0 && activeAccountId) {
        ...
        for (const id of ids) {
          ...
          if (t?.isMuted) {
            ...
            useThreadsStore.getState().updateThread(id, { isMuted: false });
          } else {
        ...
        if (thread) {
          if (thread.isMuted) {
            ...
            useThreadsStore.getState().updateThread(selectedId, { isMuted: false });
          } else {
