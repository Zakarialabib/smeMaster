...
import { freshTestDb, runMigrations, getTestAccountId, seedAccount, getTestMessages, getTestThreads, createDbInvokeHandlers, MockTauriDb } from "./setup";
...
vi.mock("@features/mail/services/gmail/client");

vi.mock("@features/settings/services/notifications/notificationManager", () => ({
...
vi.mock("@features/mail/services/categorization/ruleEngine", () => ({
...
vi.mock("@features/mail/services/smartLabels/smartLabelManager", () => ({
...
vi.mock("@features/mail/services/imap/tauriCommands", () => ({
...
describe("Integration: Sync", () => {
  // Integration tests run real SQLite migrations + IMAP JWZ-threading simulations,
  // which are legitimately slower than unit tests; allow headroom under parallel load.
  ...
  describe("Test #3: Gmail full sync cycle", () => {
    it("syncs labels, threads, and messages into the DB", async () => {
      ...
      const { syncLabels, initialSync } = await import("@features/mail/services/gmail/sync");
      const { getGmailClient } = await import("@features/mail/services/gmail/tokenManager");
      ...
      const account = await (await import("@features/accounts/db/accounts")).getAccount(getTestAccountId());
  ...
  describe("Test #4: IMAP full sync with JWZ threading", () => {
    ...
    it("JWZ-threads IMAP messages using References/In-Reply-To headers", async () => {
      ...
      const { imapInitialSync } = await import("@features/mail/services/imap/imapSync");
