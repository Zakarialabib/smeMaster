...
describe("Integration: Filters", () => {
  ...
  describe("Test #6: Apply filter rule during sync", () => {
    it("matches a message against filter criteria and applies label action", async () => {
      const { insertFilter, getFilterRuleById } = await import("@features/mail/db/filters");
      ...
      const { messageMatchesFilter } = await import("@features/mail/services/filters/filterEngine");
      ...
      const { computeFilterActions } = await import("@features/mail/services/filters/filterEngine");
      ...
      const { applyFiltersToMessages } = await import("@features/mail/services/filters/filterEngine");
    ...
    it("applies archive action when filter matches", async () => {
      const { insertFilter } = await import("@features/mail/db/filters");
      await insertFilter({
        accountId: getTestAccountId(),
