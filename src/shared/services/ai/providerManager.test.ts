...
vi.mock("./providers/lmstudioProvider", () => ({
  createLMStudioProvider: vi.fn(() => createMockAiProvider("lmstudio response")),
  clearLMStudioProvider: vi.fn(),
}));
...
describe("providerManager", () => {
  ...
  describe("getActiveProviderName", () => {
    ...
    it("returns lmstudio when ai_provider is lmstudio", async () => {
      mockGetSetting.mockResolvedValue("lmstudio");
      expect(await getActiveProviderName()).toBe("lmstudio");
    });
  ...
  describe("getActiveProvider", () => {
    ...
    it("creates lmstudio provider with server url and model", async () => {
      mockGetSetting.mockImplementation(async (key: string) => {
        if (key === "ai_provider") return "lmstudio";
        if (key === "lmstudio_server_url") return "http://localhost:1234";
        if (key === "lmstudio_model") return "llama3.2";
        return null;
      });
    ...
    it("caches lmstudio provider and reuses on subsequent calls", async () => {
      mockGetSetting.mockImplementation(async (key: string) => {
        if (key === "ai_provider") return "lmstudio";
        if (key === "lmstudio_server_url") return "http://localhost:1234";
        if (key === "lmstudio_model") return "llama3.2";
      ...
      await getActiveProvider();
      await getActiveProvider();
      expect(createLMStudioProvider).toHaveBeenCalledTimes(1);
    });
  ...
  describe("isAiAvailable", () => {
    ...
    it("returns true for lmstudio when server url is configured", async () => {
      mockGetSetting.mockImplementation(async (key: string) => {
        if (key === "ai_enabled") return "true";
        if (key === "ai_provider") return "lmstudio";
        if (key === "lmstudio_server_url") return "http://localhost:1234";
    ...
    it("returns false for lmstudio when server url is not configured", async () => {
      mockGetSetting.mockImplementation(async (key: string) => {
        if (key === "ai_enabled") return "true";
        if (key === "ai_provider") return "lmstudio";
        return null;
