 ...
 * Uses the Rust-backed db_execute_search_query command.
 */
  ...
  if (accountId) {
    const results = await executeSearchQuery(
      `SELECT
       ...
       ORDER BY rank
       LIMIT $3`,
      [ftsQuery, accountId, limit] as unknown[],
    );
    return results as unknown as SearchResult[];
  }
...
export async function unifiedSearch(
  ...
  // ── Helper: run a search query and return typed results, or [] on failure ──
  async function searchBlock(sql: string, params: unknown[]): Promise<UnifiedSearchResult[]> {
    try {
  ...
  if (accountId) {
    const messageResults = await searchBlock(
      `SELECT
        'message' as type,
        m.id,
  ...
  if (accountId) {
    const fileResults = await searchBlock(
      `SELECT
        'file' as type,
        cf.id,
  ...
  if (accountId) {
    const taskResults = await searchBlock(
      `SELECT
        'task' as type,
        t.id,
        t.title,
  ...
  if (accountId) {
    const contactResults = await searchBlock(
      `SELECT
        'contact' as type,
        c.id,
